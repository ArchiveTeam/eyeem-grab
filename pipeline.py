...
LUA_SHA1 = get_hash(os.path.join(CWD, 'eyeem.lua'))
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'p':
                if not banned_checked:
                    ...
                    #for _ in range(30):
                    #    if requests.get(
                    #        'https://www.eyeem.com/p/'+item_value,
                    #        timeout=10,
                    #        headers=headers
                    ...
                    #        break
                    #else:
                    #    Exception('Could not find workaround.')
                    #for k, v in headers.items():
                    #    print('Using', k, v)
                    #    wget_args.extend(['--header', '{}: {}'.format(k, v)])
                    banned_checked = True
                wget_args.extend(['--warc-header', 'eyeem-photo: '+item_value])
                wget_args.append('https://www.eyeem.com/p/'+item_value)
            #elif item_type == 'u':
            #    wget_args.extend(['--warc-header', 'eyeem-user: '+item_value])
            #    wget_args.append('https://www.eyeem.com/u/'+item_value)
            #elif item_type == 'tag':
            #    wget_args.extend(['--warc-header', 'eyeem-tag: '+item_value])
            #    wget_args.append('https://www.eyeem.com/search/pictures/'+item_value)
            #elif item_type == 'asset':
            #    url = 'https://' + item_value
            #    wget_args.extend(['--warc-header', 'eyeem-asset: '+url])
