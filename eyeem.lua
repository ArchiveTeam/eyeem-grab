...
local item_patterns = {
  ["^https?://[^/]*eyeem%.com/p/([0-9]+)"]="p",
  ["^https?://[^/]*eyeem%.com/u/([0-9a-zA-Z_]+)"]="u",
  ["^https?://[^/]*eyeem%.com/search/pictures/([^%?&;/]+)"]="tag"
}
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local function queue_sizes(base, w, h)
    for _, tile in pairs({252, 285, 213}) do
      local size = math.ceil(w/h*tile)
      if size < 100 then
        check(base .. tostring(size))
      ...
      elseif size >= 400 then
        ...
        check(base .. tostring(math.min(1280, size)))
      end
    end
  end
  ...
  if allowed(url)
    ...
    if item_type == "p" then
      ...
      if string.match(url, "/p/[0-9]+$") then
        ...
        local photo_url_public_id = string.match(p["photoUrlPublic"], "^https?://cdn%.eyeem%.com/thumb/([0-9a-f]+)/")
        if not photo_url_public_id then
          error("Unexpected public photo URL structure " .. p["photoUrlPublic"] .. ".")
        end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["eyeem-pcxzauj36vbq39ds"] = discovered_items
  }) do
